...
function Step({ cx, sub }: { cx: number; sub: string }) {
  return (
    <g>
      ...
      <line x1={cx} y1={138} x2={cx} y2={104} stroke="var(--color-inksoft)" strokeWidth={1.6} markerEnd="url(#rnn-arrow-ink)" />
      <text x={cx} y={94} textAnchor="middle" fontSize={14} style={MONO} fill="var(--color-inksoft)">
        y{sub}
      </text>

      {/* the cell */}
      <rect x={cx - 43} y={140} width={86} height={72} rx={12} fill="none" stroke="var(--color-sky)" strokeWidth={1.8} />
      <text x={cx} y={184} textAnchor="middle" fontSize={22} fontWeight={600} style={DISPLAY} fill="var(--color-ink)">
        A
      </text>

      {/* input xₜ, arrow up into the cell */}
      <line x1={cx} y1={250} x2={cx} y2={214} stroke="var(--color-inksoft)" strokeWidth={1.6} markerEnd="url(#rnn-arrow-ink)" />
      <text x={cx} y={270} textAnchor="middle" fontSize={14} style={MONO} fill="var(--color-inksoft)">
        x{sub}
      </text>
    </g>
  )
      ...
      >
        <defs>
          <marker id="rnn-arrow-ink" markerWidth="9" markerHeight="9" refX="6" refY="3" orient="auto">
            <path d="M0,0 L6,3 L0,6 Z" fill="var(--color-inksoft)" />
          </marker>
        ...
        <text x={0} y={20} fontSize={12} fontWeight={600} letterSpacing={2} style={DISPLAY} fill="var(--color-inksoft)">
          ONE RECURRENT CELL, UNROLLED IN TIME
        </text>

        {/* ── Folded form ─────────────────────────────── */}
        <line x1={105} y1={138} x2={105} y2={104} stroke="var(--color-inksoft)" strokeWidth={1.6} markerEnd="url(#rnn-arrow-ink)" />
        <text x={105} y={94} textAnchor="middle" fontSize={14} style={MONO} fill="var(--color-inksoft)">
          yₜ
        </text>
        <rect x={62} y={140} width={86} height={72} rx={12} fill="none" stroke="var(--color-sky)" strokeWidth={1.8} />
        <text x={105} y={184} textAnchor="middle" fontSize={22} fontWeight={600} style={DISPLAY} fill="var(--color-ink)">
          A
        </text>
        <line x1={105} y1={250} x2={105} y2={214} stroke="var(--color-inksoft)" strokeWidth={1.6} markerEnd="url(#rnn-arrow-ink)" />
        <text x={105} y={270} textAnchor="middle" fontSize={14} style={MONO} fill="var(--color-inksoft)">
          xₜ
        </text>
